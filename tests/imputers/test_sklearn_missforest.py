...
from hyperimpute.plugins.imputers.plugin_sklearn_missforest import plugin
...
def from_api() -> ImputerPlugin:
    return Imputers().get("sklearn_missforest")
...
def test_sklearn_missforest_plugin_sanity(test_plugin: ImputerPlugin) -> None:
    assert test_plugin is not None


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_serde()])
def test_sklearn_missforest_plugin_name(test_plugin: ImputerPlugin) -> None:
    assert test_plugin.name() == "sklearn_missforest"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_serde()])
def test_sklearn_missforest_plugin_type(test_plugin: ImputerPlugin) -> None:
    assert test_plugin.type() == "imputer"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_serde()])
def test_sklearn_missforest_plugin_hyperparams(test_plugin: ImputerPlugin) -> None:
    assert len(test_plugin.hyperparameter_space()) == 3
    assert test_plugin.hyperparameter_space()[0].name == "n_estimators"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_serde()])
def test_sklearn_missforest_plugin_fit_transform(test_plugin: ImputerPlugin) -> None:
    plugin = test_plugin
