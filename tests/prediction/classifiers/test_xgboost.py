...
from hyperimpute.plugins.prediction.classifiers.plugin_xgboost import plugin
...
def from_api() -> PredictionPlugin:
    return Predictions().get("xgboost")
...
def test_xgboost_plugin_sanity(test_plugin: PredictionPlugin) -> None:
    assert test_plugin is not None


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_xgboost_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "xgboost"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_xgboost_plugin_type(test_plugin: PredictionPlugin) -> None:
...
@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_xgboost_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
...
@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_xgboost_plugin_score(test_plugin: PredictionPlugin) -> None:
    X, y = load_iris(return_X_y=True, as_frame=True)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
