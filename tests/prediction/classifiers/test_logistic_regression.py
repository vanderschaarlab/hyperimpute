...
from hyperimpute.plugins.prediction.classifiers.plugin_logistic_regression import plugin
...
def from_api() -> PredictionPlugin:
    return Predictions().get("logistic_regression")
...
def test_logistic_regression_plugin_sanity(test_plugin: PredictionPlugin) -> None:
    assert test_plugin is not None


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_logistic_regression_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "logistic_regression"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_logistic_regression_plugin_type(test_plugin: PredictionPlugin) -> None:
...
@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_logistic_regression_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
