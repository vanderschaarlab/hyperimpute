...
from hyperimpute.plugins.prediction.classifiers.plugin_kneighbors import plugin
...
def from_api() -> PredictionPlugin:
    return Predictions().get("kneighbors")
...
def test_kneighbors_plugin_sanity(test_plugin: PredictionPlugin) -> None:
    assert test_plugin is not None


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_kneighbors_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "kneighbors"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_kneighbors_plugin_type(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.type() == "prediction"
    assert test_plugin.subtype() == "classifier"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_kneighbors_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
    assert len(test_plugin.hyperparameter_space()) == 5


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_kneighbors_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
