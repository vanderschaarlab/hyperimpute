...
from hyperimpute.plugins.prediction.regression.plugin_svm import plugin
...
def from_api() -> PredictionPlugin:
    return Predictions(category="regression").get("svm")
...
def test_svm_plugin_sanity(test_plugin: PredictionPlugin) -> None:
    assert test_plugin is not None


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_svm_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "svm"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_svm_plugin_type(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.type() == "prediction"
    assert test_plugin.subtype() == "regression"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_svm_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
    assert len(test_plugin.hyperparameter_space()) == 3


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_pickle()])
def test_svm_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
    X, y = load_diabetes(return_X_y=True)

    score = evaluate_regression(test_plugin, X, y)

    assert score["clf"]["rmse"][0] < 6000
