...
from hyperimpute.plugins.prediction.regression.plugin_random_forest_regressor import (
    plugin,
)
...
def from_api() -> PredictionPlugin:
    return Predictions(category="regression").get("random_forest_regressor")
...
def test_random_forest_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "random_forest_regressor"
