...
from hyperimpute.plugins.prediction.regression.plugin_gpboost_regressor import plugin
...
def from_api() -> PredictionPlugin:
    return Predictions(category="regression").get("gpboost_regressor")
...
def test_gpboost_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "gpboost_regressor"
